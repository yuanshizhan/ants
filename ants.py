...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            other_ant = self.ant
            if other_ant != None:
                if other_ant.container==True and other_ant.can_contain(insect):
                    other_ant.contain_ant(insect)
                    self.ant = other_ant
                elif insect.container and insect.can_contain(other_ant):
                    insect.contain_ant(other_ant)
                    self.ant = insect
            else:
    ...
    def remove_insect(self, insect):
        ...
        if type(self.ant) != QueenAnt or self.ant.imposter:
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        ...
        self.imposter = QueenAnt.is_imposter
    ...
    def action(self, colony):
        ...
        colony.queen = QueenPlace(self.place, end)
        if not self.imposter:
            ThrowerAnt.action(self, colony)
            while position.exit != None:
                if position.exit.ant != None:
                    ants_behind += [position.exit.ant]
                position = position.exit
            for ant in ants_behind:
                if ant.container and ant.ant != None and not ant.ant.doubled:
                    ant.ant.damage = ant.ant.damage*2
                    ant.ant.doubled = True
                elif not ant.doubled:
